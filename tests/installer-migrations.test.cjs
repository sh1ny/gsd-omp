...
const _opencodeMigration = null; // removed-runtime stub
...
test('records known generated agent artifacts so profile cleanup can remove them', () => {
  ...
  try {
    ...
    assert.deepEqual(result.blocked.map((action) => action.relPath), ['agents/gsd-executor.toml', 'agents/gsd-local-experiment.md']);
...
test('shipped installer-migration checksums are locked to a committed baseline (issue #670 guardrail)', () => {
  ...
  const EXPECTED_CHECKSUMS = {
    ...
    // Migration 005 + 006 (codex-legacy-hooks-json, opencode-baseline-commands-dir,
    // pi-extension-cjs-to-js) were removed when this fork went OMP-only — no
    // shipped migrations reference removed runtimes.
  };
